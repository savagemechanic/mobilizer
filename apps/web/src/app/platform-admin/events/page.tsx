...
import { Calendar, MapPin, Users, Eye, Trash2, Search, Filter, X, Building2 } from 'lucide-react'
import { GET_PLATFORM_EVENTS, GET_MOVEMENTS, GET_PLATFORM_ORGANIZATIONS } from '@/lib/graphql/queries/platform-admin'
...
export default function PlatformAdminEventsPage() {
  ...
  // Query events
  const { data: eventsData, loading: eventsLoading, error: eventsError, refetch } = useQuery(GET_PLATFORM_EVENTS, {
    variables: {
      limit: pageSize,
  ...
  const { data: orgsData } = useQuery(GET_PLATFORM_ORGANIZATIONS, {
    variables: {
      filter: selectedMovement !== 'all' ? { movementId: selectedMovement } : {},
      limit: 500,
      offset: 0,
    },
  })
  ...
  const filteredEvents = events.filter((event) => {
    ...
    // Movement filter
    if (selectedMovement !== 'all' && event.organization?.movementId !== selectedMovement) {
      return false
    }
    ...
    // Location filters
    if (selectedCountry && event.organization?.country?.id !== selectedCountry) {
      return false
    }
    if (selectedState && event.organization?.state?.id !== selectedState) {
      return false
    }
    if (selectedLGA && event.organization?.lga?.id !== selectedLGA) {
      return false
    }
    if (selectedWard && event.organization?.ward?.id !== selectedWard) {
      return false
    }
    if (selectedPollingUnit && event.organization?.pollingUnit?.id !== selectedPollingUnit) {
      return false
    }
  ...
  const getLocationDisplay = (event: Event) => {
    ...
    const parts = []
    if (org.pollingUnit?.name) parts.push(org.pollingUnit.name)
    else if (org.ward?.name) parts.push(org.ward.name)
    else if (org.lga?.name) parts.push(org.lga.name)
    else if (org.state?.name) parts.push(org.state.name)
    else if (org.country?.name) parts.push(org.country.name)
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left p-3 font-semibold">Movement</th>
                      ...
                      <tr key={event.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-3">
                          <span className="text-sm truncate max-w-[120px] block">
                            {event.organization?.movement?.name || 'N/A'}
                          </span>
                        </td>
                        <td className="p-3">
